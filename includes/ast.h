...
/*   Updated: 2025/11/08 17:06:09 by acoronad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Nota: las listas de redirecciones se encadenan por `next` */
struct s_ast
{
	t_node_type	type;
	struct s_ast	*next;
	union
	{
		struct {
			char		**argv;
			struct s_ast	*redirections; /* lista N_REDIR por `next` */
		}	cmd;
		struct {
			char		*filename;
			char		*delimiter;
			...
			int		heredoc_quoted; /* 1 si delimitador venía citado */
		}	redir;
		struct {
			struct s_ast	*left;
			struct s_ast	*right;
