...
/*   Updated: 2025/07/03 18:17:16 by acoronad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (*cur && is_redirection(*cur))
	{
		...
		if (!file_tok || file_tok->type != T_WORD || rtype == REDIR_INVALID)
			return (ft_dprintf(2,
				...
				file_tok ? file_tok->value : "newline"), 0);
		...
		{
			fd = heredoc_read(file_tok->value);
			if (fd == -1)
				return (0);
			new_redir = ast_new_redir(ft_strdup(file_tok->value),
							NULL, rtype, fd);
		}
		else
			new_redir = ast_new_redir(ft_strdup(file_tok->value),
		...
		next_token(cur);
		if (!*head)
			*head = new_redir, *tail = new_redir;
		else
			(*tail)->bin.right = new_redir, *tail = new_redir;
	}
	return (1);
}
