...
/*   Updated: 2025/11/08 17:02:38 by acoronad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int op_match_len1(const char *s, t_token_type *type, int *len)
{
    if (!s) return (0);
    if (*s == '|')
		return (op_set(type, len, T_PIPE, 1), 1);
    if (*s == '&')
		return (op_set(type, len, T_BG, 1), 1);
    if (*s == ';')
		return (op_set(type, len, T_SEMI, 1), 1);
    if (*s == '<')
		return (op_set(type, len, T_REDIR_IN, 1), 1);
    if (*s == '>')
		return (op_set(type, len, T_REDIR_OUT, 1), 1);
    if (*s == '(')
		return (op_set(type, len, T_LPAREN, 1), 1);
    if (*s == ')')
		return (op_set(type, len, T_RPAREN, 1), 1);
    return (0);
}
